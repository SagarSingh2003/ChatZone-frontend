...
export default function RegularOptions({ x , y ,  setShowRegularOptions , setRefreshServerAndRoomData}){
              ...
              <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                  ...
                  <div class="p-4 md:p-5 text-center">
                      ...
                      <button onClick={async () => {
                          ...
                          if (value) { console.log("recieved response " , value); setRefreshServerAndRoomData(c => c + 1); setShowSpinner(false); setMessage(value); setTimeout(() => {setMessage(null) ; setShowRegularOptions({show: false})} , 2000)};
  ...
  : (
    ...
    <section className="float-left absolute w-[200px]" style={{top : `${Number(y -20)}px` , left : `${Number(x) + 40}px`}}>
              
        <section className="w-[100%] float-left absolute z-[2] "  >
          <section className="text-white p-[15px] border-b rounded flex items-center justify-center bg-[#12151D] border-b-[#1D202F] hover:bg-[#1D202F] hover:cursor-pointer">Message Privately </section>
          <section onClick={() => {setShowRegularOptions(false)}} className="text-white flex items-center  bg-[#D32529]  hover:bg-[#991B1B] rounded justify-center p-[15px] rounded  hover:cursor-pointer ">Cancel</section>
        </section>
    </section>
    )
